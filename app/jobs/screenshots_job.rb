class ScreenshotsJob < ActiveJob::Base

  # Update screenshots for pages without screenshots or that have shots older than 30 minutes
  # We only process 5 new screenshots per job
  def perform
    updated = 0
    now = Time.new
    Page.all.each do |page|
      if !page.screenshot.exists? || page.screenshot_updated_at.nil? || (now - page.screenshot_updated_at > 1800)
